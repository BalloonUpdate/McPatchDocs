...
McPatch不同于老版本的JarClient截然不同，使用过程中请务必仔细阅读文档
...
先决条件
> Java已正确安装。验证安装方法：cmd运行`java -version`
> 已经下载好了3个必要jar文件。

1. 在您能找得到的地方新建一个目录，取一个你喜欢的名字，并打开这个目录。
2. 把`McPatchManage.jar`和`MiniHttpServer.jar`复制进去。
3. 点击一下窗口空白处
4. 按住shift，在窗口空白处右击，选择在此处打开powershell窗口。
5. 输入以下命令（不要执行）:
~~~powershell
java -jar McPatchManage.jar && java -jar MiniHttpServer.jar
~~~
...
然后powershell执行刚才打好的命令，输入`1`来开始创建第一个更新包。版本号可以随意决定，推荐您使用1.0-a，按下回车即可准备打包
...
一切妥当之后，输入`y`开始打包。首次打包内容较大，可能会花费相当多的时间，请耐心等待，如果出现内存不够，请在刚才的命令里插入`-Xmx?G`选项。

待到出现“创建版本完成”的字样后，就说明打包成功了（更新包文件会保存在public目录下）

!> 注意：已发布的版本切勿手动删除，会导致后续更新全部出错！如需撤回版本请阅读[版本发错了怎么办](#版本发错了怎么办)

### 2.开启自带的MiniHttpServer

上面的命令会帮你在打包结束后启动http server，启动成功后复制API地址
...
> 如您在中国大陆公有云部署，那么意味着您需要备案，否则可能会被停止访问。

<p><div class="blinkred">注意在本机调试好后，如果不将API地址里的localhost替换成你服务器的IP，玩家的客户端一定会提示`连接被拒绝`或者`连接超时`</div></p>
...
你只需要将public里的文件上传到服务器可以公开访问的地方（必须确保所有文件都可正常访问**），然后按照Config.yml说明编写你的服务器地址，例如
```
https://your.upgrade.server/your/public/flie/location/
```
如果您不太懂这些又想甩掉没什么功能的MimiHttpServer，请移步高级教程（施工中）

### 版本号并非判断新旧的依据

版本号内容只是给人类看的，版本号内容含义对程序没有意义。

程序判断版本的依据是版本号在 mc-patch-versions.txt 的先后顺序，所以版本号可以随意填写。

我们推荐这样填写： 正式版本.补丁版本-发行状态，例如：
> 0.1-aplha
> 5.2-beta
> 3.7-release

至于哪个版本更新，哪个版本更旧？可以亲自打开“版本列表文件(mc-patch-versions.txt)”一看便知（**不要改动，后果自负。**）
...
1. 打开public/mc-patch-versions.txt文件，将错误版本那一行连带后面所有的行都删除掉（**不要改动其他行，后果自负**）
...
### 对配置文件和版本号文件进行base64编码

> 此特性仅McPatchClient 1.0.11或更高版本支持

将内容base64编码后以非明文保存，不能起到安全防护的作用，只能给有心者带来一些小麻烦。

编码配置文件（config.yml）：将整个配置文件内容复制后使用Base64进行编码，然后删掉原有内容，将编码后的内容粘贴进去，然后在文件的开头添加一个英文冒号`:`用来告诉程序配置文件被编码了，使用之前要先解码。

加密版本号文件（mc-patch-version.txt）：将整个版本号文件内容复制后使用Base64进行编码，然后删掉原有内容，将编码后的内容粘贴进去，然后在文件的开头添加一个英文冒号`:`用来告诉程序配置文件被编码了，使用之前要先解码。

注意：配置文件和版本号文件编码后，不会影响日志文件里的账号密码信息的显示，如果在意，可以在配置文件里禁用日志文件生成。
