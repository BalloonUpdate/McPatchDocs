...
`问题解答`：windows环境下不可以。原因：Windows会锁定处于读写状态的文件，故无法自更新。

`小小提示`：虽然windows不可以更新McPatchClient本身，但是更新配置文件`config.yml`是允许的，但缺点是第二次启动时新的配置文件才会生效。如您真的需要，可以使用powershell一句话获取您的脚本并执行，下载并覆写McPatchClient，并在启动器设置里打开游戏运行完毕后执行一句话。
...
`解决方法`：请自行确认远程主机是否可信，如果可信，请更新SFTP源的服务器字符串中的指纹部分，如果您的主机在中国大陆，并且您不信任这个密钥，建议报警。
